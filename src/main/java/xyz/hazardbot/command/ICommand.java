...
public abstract class ICommand {
    ...
    public void onCommand(Message message, TextChannel textChannel, String commandUsed, String[] args) {
        ...
        if (minArgs > args.length) {
            ...
            EmbedBuilder eb = new EmbedBuilder().setTitle("Error!").setDescription("Invalid command usage")
                    .setColor(Color.RED).addField("Valid Usage", format);
        ...
        if (getRequiredAccessLevel() > 0) {
            ...
            if (ostc.isPresent()) {
                ...
                if (userAccessLevel < requiredLevel) {
                    EmbedBuilder eb = new EmbedBuilder().setTitle("Error!").setDescription("Access Denied!")
                            .setColor(Color.RED);
                    textChannel.sendMessage(eb);
                }
            } else return;
        ...
        try {
            ...
            EmbedBuilder eb = new EmbedBuilder().setTitle("Error!")
                    .setDescription("An exception occured, tell a bot owner!").setColor(Color.RED)
    ...
    public Message getOriginalDiscordMessage() {
        return message;
    }
